...
int BIN_loader(const char *filename) {
  ...
  iprintf("BIN_loader\n");
	...
	if (machine_type == MACHINE_5200) {
		iprintf("binload: can't run Atari programs directly on the 5200");
		return FALSE;
	}
	bin_file = fopen(filename, "rb");
	if (bin_file == NULL) {	/* open */
		iprintf("binload: can't open \"%s\"", filename);
		return FALSE;
	}
	...
	fclose(bin_file);
	bin_file = NULL;
	iprintf("binload: \"%s\" not recognized as a DOS or BASIC program", filename);
	return FALSE;
}
