/*
 * DEVICES.C contains the emulation of various peripheral devices on the Atari.
* 
...
{
	UWORD bufadr;
	for (bufadr = dGetWord(Devices_ICBALZ); ; bufadr++) {
...
static void Devices_H_DiskInfo(void)
{
	...
	CopyToMem(info, (UWORD) dGetWord(Devices_ICBLLZ), 16);
...
static void Devices_H_ToAbsolutePath(void)
{
	...
	bufadr = dGetWord(Devices_ICBLLZ);
	if (atari_path[0] != '\0') {
...
static void Devices_RestoreHandler(UWORD address, UBYTE esc_code)
{
	...
	dCopyToMem(Atari800_machine_type == Atari800_MACHINE_XLXE
...
void Devices_UpdatePatches(void)
{
	...
	if (Devices_enable_b_patch) {
		...
		/* set handler table */
		dPutWord(B_TABLE_ADDRESS + Devices_TABLE_OPEN, B_PATCH_OPEN - 1);
		dPutWord(B_TABLE_ADDRESS + Devices_TABLE_CLOS, B_PATCH_CLOS - 1);
		dPutWord(B_TABLE_ADDRESS + Devices_TABLE_READ, B_PATCH_READ - 1);
		dPutWord(B_TABLE_ADDRESS + Devices_TABLE_WRIT, B_PATCH_WRIT - 1);
		dPutWord(B_TABLE_ADDRESS + Devices_TABLE_STAT, B_PATCH_STAT - 1);
		dPutWord(B_TABLE_ADDRESS + Devices_TABLE_SPEC, B_PATCH_SPEC - 1);
		dPutWord(B_TABLE_ADDRESS + Devices_TABLE_INIT, B_PATCH_INIT - 1);
		/* set patches */
		...
		/* fill memory area used for table and patches with 0xff */
		dFillMem(B_DEVICE_BEGIN, 0xff, B_DEVICE_END - B_DEVICE_BEGIN + 1);
	}
}
